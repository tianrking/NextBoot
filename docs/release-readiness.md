...
Status: **v0.1.0-rc.11 prerelease candidate**. Updated 2026-09-13. Its release gate uses a same-size 7GB image mounted by the native Windows exFAT driver and a full-range write verification through a fixed VHD wrapper. This is not a stable or physical-hardware certification.

Physical Intel UEFI evidence on 2026-09-13 confirmed first-boot GPT/exFAT growth on removable SD media, then exposed a raw fallback scan that continued into internal disks when firmware supplied no SimpleFileSystem handles for NEXTDATA. RC.11 restricts that fallback to the physical disk which loaded NextBoot; physical menu and ISO handoff remain required before stable promotion.
...
by the target user flow. The changes below are prepared for `v0.1.0-rc.11` and
...
v0.0.3 的发布介质缺少此前真实 ISO 测试所使用的兼容运行资源。`v0.1.0-rc.11`
...
| Synthetic boot paths | Full QEMU matrix and two-image recovery passed at 922a10f; Alpine, Debian, Fedora, Ubuntu and Kali independent release-media boot checks passed | Synthetic Btrfs fixtures are not ordinary Btrfs support; final release artifact must be checked again |
...
| Fedora Workstation 44 x64 | Actual release builder → GNOME Display Manager in CI at 6eaab56 | Complete desktop session, installation and physical machine verification |
