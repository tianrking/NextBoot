...
`v0.1.0-rc.16`; it must not be interpreted as completed-installation or
physical-hardware certification.

v0.0.3 的发布介质缺少此前真实 ISO 测试所使用的兼容运行资源。`v0.1.0-rc.16`
...
| Synthetic boot paths | Full QEMU matrix and two-image recovery passed on release commit `7a4d05e`; Alpine, Debian, Fedora, Ubuntu and Kali independent release-media boot checks passed after the scanner fix on `aeceeac` (the loader/runtime are unchanged in rc.15) | Synthetic Btrfs fixtures are not ordinary Btrfs support; physical hardware still needs evidence |
