...
- Raw `.img` and fixed VHD virtual hard-disk boot smoke coverage with an inner
  GPT/FAT32 ESP.
...
- Broader image types: raw IMG and fixed VHD have virtual hard-disk boot smoke
  coverage now; dynamic VHD, VHDX, and VDI scanning exists in the product
  direction, but boot coverage is still thinner than ISO/WIM/raw IMG/fixed VHD
  flows.
