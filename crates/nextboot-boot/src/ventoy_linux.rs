...
const CPIO_FINAL_ALIGNMENT: usize = 512;
...
pub fn build_ventoy_linux_initrd(
    ...
    let mut builder = NewcArchiveBuilder::new();
...
impl NewcArchiveBuilder {
    ...
    fn append_archive_without_trailer(
        &mut self,
        archive: &[u8],
    ...
    fn add_file(&mut self, name: &str, contents: &[u8]) -> Result<(), VentoyLinuxInitrdError> {
        self.add_entry(name, CPIO_FILE_MODE, contents)
    }

    fn finish(mut self) -> Result<Vec<u8>, VentoyLinuxInitrdError> {
...
fn parse_hex_field(header: &[u8], offset: usize) -> Result<u32, VentoyLinuxInitrdError> {
    let field = header
        .get(offset..offset + 8)
        .ok_or(VentoyLinuxInitrdError::InvalidArchive)?;
    let mut value = 0u32;
