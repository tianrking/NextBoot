...
# 用 FAT32 Data 分区覆盖固定 SSD/NVMe 上的旧式兼容布局
./scripts/run-qemu.sh --bus nvme --layout split --data-fs fat32 --sector-size 4096 --smoke-efi-iso
...
# 用 4K USB SSD/移动硬盘盒 + FAT32 Data 分区测试
./scripts/run-qemu.sh --bus usb --layout split --data-fs fat32 --sector-size 4096 --smoke-efi-iso
...
# 用 4K virtio 固定盘路径测试
./scripts/run-qemu.sh --bus virtio --layout split --data-fs exfat --sector-size 4096 --smoke-efi-iso
...
./scripts/run-qemu.sh --bus nvme --layout split --data-fs ntfs --sector-size 4096 --smoke-efi-iso
...
# 用 4K USB SSD/移动硬盘盒 + UDF Data 分区测试
./scripts/run-qemu.sh --bus usb --layout split --data-fs udf --sector-size 4096 --smoke-efi-iso
...
真实启动、USB 512 split/FAT32 真实启动，以及 SD 512 split/FAT32、exFAT、NTFS、UDF 带小 ISO 的镜像
生成与校验。`NEXTBOOT_FULL_QEMU_MATRIX=1 ./scripts/qemu-smoke-matrix.sh` 会继续覆盖
virtio、virtio 4K/FAT32、virtio 4K/exFAT、virtio 4K/NTFS、virtio 4K/UDF、virtio 4K/ext2/ext3/ext4、USB 4K/FAT32、USB 4K/exFAT、USB 4K/NTFS、USB 4K/UDF、USB 4K/ext2/ext3/ext4、SATA/FAT32、SATA/exFAT、SATA/NTFS、SATA/UDF、NVMe 4K/FAT32、NVMe 4K/NTFS、NVMe/UDF、NVMe/ext2/ext3/ext4、NVMe/XFS、NVMe 512B/XFS、NVMe/XFS VLNK、NVMe raw IMG、NVMe fixed/dynamic VHD、NVMe VHDX、NVMe sparse/partially-present VHDX、parent-required VHDX 负向路径、NVMe dynamic/static/sparse/discarded VDI、parent-required VDI 负向路径和 NVMe/ext4 Linux 插件载荷。SD 目前限制为 512B 扇区，因为 QEMU `sd-card` 设备没有提供和
