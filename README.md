...
- ✅ 支持 FAT32/exFAT/ext2/ext3/ext4/NTFS/UDF/ISO9660 文件系统
- ✅ GPT 分区表解析
- ✅ 虚拟 Block IO 设备模拟
- ✅ 图形化菜单界面 (GOP)
- ✅ Linux 发行版引导支持
...
│   ├── nextboot-fs/       # 文件系统模块 (FAT32, exFAT, ext2/3/4, NTFS, UDF, ISO9660)
...
`run-qemu.sh` 会直接创建 GPT/FAT32/exFAT/ext2/ext3/ext4/NTFS/UDF 磁盘镜像，并支持 `virtio`、`nvme`、`sata`、
...
固定盘上“引导分区与镜像分区分离”的真实部署路径；`--data-fs exfat|ext2|ext3|ext4|fat32|ntfs|udf` 可覆盖
默认写盘布局、Linux ext 系列数据盘、FAT32 兼容布局、NTFS 大文件布局和 UDF 数据盘布局。生成后脚本会调用
`verify-qemu-image.py` 校验 GPT CRC、分区布局、FAT32/exFAT/ext2/3/4/NTFS/UDF 目录、`BOOTX64.EFI`、
...
virtio、SATA/NTFS、NVMe/UDF、NVMe/ext3 和 NVMe/ext4 Linux 插件载荷。SD 目前限制为 512B 扇区，因为 QEMU `sd-card` 设备没有提供和
...
启动文件；第 2 分区是 Data 分区，默认 exFAT，也可选择 FAT32、ext2/ext3/ext4、NTFS 或 UDF，用来存放
`/ISO` 下的 ISO/WIM/VHD 文件。旧式单分区 FAT32 仍可通过 `--layout single` 生成。
在 Linux 上写入 ext/UDF Data 分区需要 `mkfs.ext2`/`mkfs.ext3`/`mkfs.ext4`/`mkudffs`。在 macOS 上写入 NTFS
