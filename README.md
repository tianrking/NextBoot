...
**Release status:** `v0.1.0-rc.9` is a prerelease. It adds a Windows-native
...
[![USB Boot Image](https://img.shields.io/badge/image-flashable%20USB%20%2F%20SSD-purple)](https://github.com/tianrking/NextBoot/releases/tag/v0.1.0-rc.9)
...
1. Download the universal image from the latest GitHub release:
   `nextboot-v0.1.0-rc.9-universal-uefi.img.xz`.
   If your flashing tool only accepts raw `.img` files, download
   `nextboot-v0.1.0-rc.9-universal-uefi.img.zip` and extract it.
2. On Windows, download `nextboot-v0.1.0-rc.9-windows-writer.ps1`, open
   **Administrator PowerShell**, run `Get-Disk` to identify the target disk,
   then run:

   ```powershell
   Unblock-File .\nextboot-v0.1.0-rc.9-windows-writer.ps1
   .\nextboot-v0.1.0-rc.9-windows-writer.ps1 `
     -ImagePath .\nextboot-v0.1.0-rc.9-universal-uefi.img -DiskNumber N
   ```
...
4. Open the visible `NEXTDATA` partition. Windows must report its filesystem as
   ...
   image with the verified Windows writer. For USB card readers and other removable SD/USB media, the writer automatically continues when Windows rejects the optional offline-disk operation; it still performs the full raw-image readback check.
...
```text
nextboot-v0.1.0-rc.9-universal-uefi.img.xz
nextboot-v0.1.0-rc.9-universal-uefi.img.zip
nextboot-v0.1.0-rc.9-windows-writer.ps1
```

Latest release: <https://github.com/tianrking/NextBoot/releases/tag/v0.1.0-rc.9>
