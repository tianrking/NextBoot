...
**发布状态：** `v0.1.0-rc.4` 是带有 QEMU 证据的预发布版本。它修复了 rc.1 和 rc.2 在 Windows
上将 `NEXTDATA` 识别为 RAW 的 exFAT 挂载缺陷；若旧版本出现该问题，请重新烧录新镜像，不要格式化
...
[![USB Boot Image](https://img.shields.io/badge/image-flashable%20USB%20%2F%20SSD-purple)](https://github.com/tianrking/NextBoot/releases/tag/v0.1.0-rc.4)
...
1. 从最新 GitHub Release 下载通用镜像：
   `nextboot-v0.1.0-rc.4-universal-uefi.img.xz`。
   如果你的烧录工具只接受 raw `.img` 文件，下载
   `nextboot-v0.1.0-rc.4-universal-uefi.img.zip` 并解压。
...
```text
nextboot-v0.1.0-rc.4-universal-uefi.img.xz
nextboot-v0.1.0-rc.4-universal-uefi.img.zip
```

最新发布：<https://github.com/tianrking/NextBoot/releases/tag/v0.1.0-rc.4>
