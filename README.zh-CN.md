...
2. Windows 下请下载 `nextboot-v0.1.0-rc.15-windows-writer.ps1`，以**管理员身份**打开
   ...
   若只是需要尽快进行一次真机启动测试，可在命令末尾加上 `-SkipFullVerification`：写入器仍会校验新建的
   `NEXTDATA` exFAT 启动记录和不可变 EFI System Partition，但跳过耗时的整盘读回。测试后应执行下方的只读 EFI
   校验；用于恢复或长期保留前则应使用默认完整校验。
...
4. 写入后打开可见的 `NEXTDATA` 分区。Windows 必须将它显示为 exFAT，且内部应有 `ISO`。若显示 RAW，不要格式化；用 Windows 校验写入器重新写入镜像。对于 USB 读卡器等可移动 SD/USB 设备，Windows 拒绝可选离线操作时写入器会继续；它会在擦除确认后清除选定的非系统磁盘以释放卷句柄，除非显式提供 `-SkipFullVerification`，否则仍会做整盘读回校验。
